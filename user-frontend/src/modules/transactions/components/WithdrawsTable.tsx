...
interface WithdrawsDataTableProps {
  data: Withdraw[];
  loading: boolean;
  total: number;
  page: number;
...
export function WithdrawsDataTable({
  data,
  loading,
  total,
  page,
  ...
  const handleStatusChange = React.useCallback(
    async (id: number, status: WithdrawStatus) => {
      try {
  ...
  const columns = React.useMemo<ColumnDef<Withdraw>[]>(
    () => [
      {
        accessorKey: "id",
        header: ({ column }) => (
              ...
              {withdraw.status === "PENDING" && (
                <>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleStatusChange(withdraw.id, "WAITING")}
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleStatusChange(withdraw.id, "REJECTED")}
              ...
              {withdraw.status === "WAITING" && (
                <>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleStatusChange(withdraw.id, "ACCEPTED")}
